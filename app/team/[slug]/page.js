        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl p-8 md:p-10">
          ...
          <div className="flex flex-col md:flex-row gap-8 items-start mb-8">
            <div className="flex-shrink-0 flex flex-col items-center gap-3">
              <div className="w-28 h-28 md:w-36 md:h-36 rounded-full bg-gradient-to-br from-violet-500 to-purple-500 flex items-center justify-center text-white font-bold text-3xl md:text-4xl">
                {member.photo ? (
                  <img src={member.photo} alt={member.name} className="w-full h-full rounded-full object-cover" />
                ) : member.initials}
              </div>
              <div className="inline-flex items-center gap-2 bg-violet-500/10 border border-violet-500/30 text-violet-300 text-xs font-medium px-3 py-1.5 rounded-full">
                {member.role}
              </div>
            </div>
          ...
          <div className="border-t border-white/10 pt-8">
            ...
            <div className="flex flex-wrap gap-2">
              {member.skills.map((skill) => (
                <span key={skill} className="bg-violet-500/10 border border-violet-500/20 text-violet-300 text-sm px-4 py-1.5 rounded-full">
                  {skill}
                </span>
              ))}
            </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {members.filter((m) => m.slug !== member.slug).map((m) => (
              <Link key={m.slug} href={`/team/${m.slug}`} className="group">
                <div className="bg-white/5 border border-white/10 rounded-xl p-4 hover:bg-white/10 hover:border-violet-500/30 transition-all text-center">
                  <div className="w-12 h-12 rounded-full bg-gradient-to-br from-violet-500 to-purple-500 flex items-center justify-center text-white font-bold text-sm mx-auto mb-2 group-hover:scale-105 transition-transform">
                    {m.photo ? (
