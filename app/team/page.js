        ...
        <div className="max-w-6xl mx-auto px-6">
          ...
          <div className="mb-6">
            <Link href={`/team/${members[0].slug}`} className="group block">
              <div className="bg-white/5 border border-white/10 rounded-2xl p-6 md:p-8 hover:bg-white/10 hover:border-violet-500/30 transition-all">
                <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
                  <div className="w-20 h-20 rounded-full bg-gradient-to-br from-violet-500 to-purple-500 flex items-center justify-center text-white font-bold text-2xl flex-shrink-0 group-hover:scale-105 transition-transform">
                    {members[0].photo ? (
                      <img src={members[0].photo} alt={members[0].name} className="w-full h-full rounded-full object-cover" />
                    ) : members[0].initials}
                  </div>
                  <div className="flex-1 text-center sm:text-left">
                    <div className="inline-flex items-center gap-2 bg-violet-500/10 border border-violet-500/30 text-violet-300 text-xs font-medium px-3 py-1 rounded-full mb-2">
                      {members[0].role}
                    </div>
                    ...
                    <div className="inline-flex items-center gap-1 text-violet-400 text-xs font-medium group-hover:gap-2 transition-all">
                      View Profile
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {members.slice(1).map((member) => (
              <Link key={member.slug} href={`/team/${member.slug}`} className="group">
                <div className="bg-white/5 border border-white/10 rounded-2xl p-6 hover:bg-white/10 hover:border-violet-500/30 transition-all h-full">
                  <div className="flex items-start gap-4">
                    <div className="w-14 h-14 rounded-full bg-gradient-to-br from-violet-500 to-purple-500 flex items-center justify-center text-white font-bold text-base flex-shrink-0 group-hover:scale-105 transition-transform">
                      {member.photo ? (
                        <img src={member.photo} alt={member.name} className="w-full h-full rounded-full object-cover" />
                      ) : member.initials}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="inline-flex items-center bg-violet-500/10 border border-violet-500/30 text-violet-300 text-xs font-medium px-2 py-0.5 rounded-full mb-1">
                        {member.role}
                      </div>
                  ...
                  <div className="mt-4 flex items-center gap-1 text-violet-400 text-xs font-medium group-hover:gap-2 transition-all">
                    View Profile
                    <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                    </svg>
